...
.defines(function() {
    EntityPirate = EntityShooter.extend({
        
        animSheet: new ig.AnimationSheet( 'media/enemies/pirate.png', 16, 16 ),
        ...
        checkAttacking: function() {
            if ( ! this.hasAttacked ) {
                if ( this.currentAnim.frame == 2 ) {
                    ...
                    var yPos = this.pos.y;
                    ig.game.spawnEntity( EntityPirateBullet, xPos, yPos );
                }
            }
        },
    ...
    ig.EntityPool.enableFor( EntityPirate );
});