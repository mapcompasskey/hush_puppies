ig.module(
    'game.entities.enemies.indian-spearman'
)
.requires(
    'impact.entity',
    'impact.entity-pool',
...
.defines(function() {
    EntityIndianSpearman = EntityShooter.extend({
        
        animSheet: new ig.AnimationSheet( 'media/indian-spearman.png', 16, 16 ),
        ...
        checkAttacking: function() {
            if ( ! this.hasAttacked ) {
                if ( this.currentAnim.frame == 2 ) {
                    ...
                    var xPos = this.pos.x;
                    var yPos = this.pos.y;
                    ig.game.spawnEntity( EntityIndianSpear, xPos, yPos );
                }
            }
        },
    ...
    ig.EntityPool.enableFor( EntityIndianSpearman );
});